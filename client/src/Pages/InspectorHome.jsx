...
const InspectorHome = () => {
  ...
  useEffect(() => {
    ...
    // Fetch complaints assigned to this inspector's station
    axios
      .get(
        `http://localhost:8080/officers/inspectors/${inspectorId}/station-complaints`
      )
      .then((res) => {
        const complaintsData = res.data.map((c) => ({
          ...c,
          priorityAssigned: c.priorityAssigned || false, // track if already assigned
        }));

        // Collect officer IDs to fetch names
        ...
        Promise.all(
          officerIds.map((id) =>
            axios
              .get(`http://localhost:8080/officers/${id}`)
              .then((res) => ({ id, name: res.data.officerName }))
          )
        ).then((results) => {
          const officerMap = {};
  ...
  const handleAssign = (complaintId, officerId) => {
    axios
      .put(
        `http://localhost:8080/complaints/${complaintId}/assign/${officerId}`
      )
      .then(() => {
        setComplaints((prev) =>
          prev.map((c) =>
        ...
        axios
          ...
          .then((res) =>
            setOfficersList((prev) => ({
              ...prev,
              [officerId]: res.data.officerName,
            }))
          );
      })
  ...
  const handlePriorityChange = (complaintId, newPriority) => {
    axios
      .put(`http://localhost:8080/complaints/${complaintId}/priority`, {
        priority: newPriority,
        priorityAssigned: true, // mark as assigned in backend
      })
      .then(() => {
        setComplaints((prev) =>
          prev.map((c) =>
            c.complaintId === complaintId
              ? { ...c, priority: newPriority, priorityAssigned: true }
              : c
          )
        );
      })
          ...
          <tbody>
            {filteredComplaints.map((c) => (
              <tr key={c.complaintId}>
                ...
                <td>
                  <select
                    value={c.priority} // last assigned value
                    disabled={c.priorityAssigned} // freeze if already assigned
                    onChange={(e) =>
                      handlePriorityChange(c.complaintId, e.target.value)
                    }
                  >
                ...
                <td>
                  {c.officerId ? (
                    ...
                    <select
                      defaultValue=""
                      onChange={(e) =>
                        handleAssign(c.complaintId, e.target.value)
                      }
                    >
